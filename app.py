...
st.sidebar.caption("Valores en MM$ (millones CLP) · Cifras acumuladas YTD")
...
def get_group_kpis() -> pd.DataFrame:
    return _fetch_group_kpis(SEL_YEAR, SEL_QTR)
...
if section == "Compañías":
    ...
    if page == "Market Overview":
        ...
        c1.metric("GWP mercado", f"MM$ {mkt_gwp / MM:,.0f}")
        ...
        fig3 = px.bar(by_type, x="Segmento", y="gwp", color="Segmento",
                      color_discrete_map={v: TYPE_COLORS[k] for k, v in TYPE_LABELS.items()},
                      labels={"gwp": "GWP (CLP)"}, title="GWP por segmento")
    ...
    # ── Zurich Focus ──────────────────────────────────────────────────────────
        ...
        for _, row in z_kpis.iterrows():
            with st.expander(row["company_name"], expanded=True):
                ...
                c1.metric("GWP", f"MM$ {row['gwp'] / MM:,.0f}")
        ...
        if _ramo_active:
            ...
            if yoy.empty:
                ...
                disp["GWP MM$"]      = (disp["gwp"] / MM).round(0)
                disp["GWP prev MM$"] = (disp["prev_gwp"] / MM).round(0)
                st.dataframe(
                    disp[["company_name", "GWP MM$", "GWP prev MM$", "gwp_yoy_pct",
                           "loss_ratio_pct", "prev_loss_ratio_pct",
    ...
    # ── Ramo Deep-dive ────────────────────────────────────────────────────────
        ...
        fig1 = px.bar(by_product, x="product_group", y="gwp",
                      ...
                      labels={"gwp": "GWP (CLP)", "product_group": ""},
    ...
    # ── YoY Trends ────────────────────────────────────────────────────────────
        ...
        all_period_kpis = pd.concat(
            [ftype(_fetch_kpis(int(r.year), int(r.quarter))) for r in periods_df.itertuples()],
            ignore_index=True,
        )
        if _ramo_active:
            ramo_all = pd.concat(
                [framo(ftype(_fetch_ramo(int(r.year), int(r.quarter)))).assign(year=int(r.year), quarter=int(r.quarter))
        ...
        if _ramo_active:
            ...
            yoy_all = ftype(q("SELECT * FROM vw_period_yoy WHERE year=? AND quarter=?", (SEL_YEAR, SEL_QTR)))
    ...
    # ── M&A Groups ────────────────────────────────────────────────────────────
        ...
        disp = g_kpis.copy()
        disp["GWP MM$"] = (disp["gwp"] / MM).round(0)
        disp["NEP MM$"] = (disp["nep"] / MM).round(0)
        st.dataframe(
            disp[["group_name", "GWP MM$", "NEP MM$", "loss_ratio_pct", "expense_ratio_pct",
        ...
        g_ms = _fetch_group_ms(SEL_YEAR, SEL_QTR)
        if True:
            if not g_ms.empty:
        ...
        g_all = q("SELECT * FROM vw_group_kpis ORDER BY year, quarter")
        ...
        fig_t = px.line(g_all, x="período", y="gwp", color="group_name", markers=True,
                        ...
                        labels={"período": "Período", "gwp": "GWP (CLP)", "group_name": "Grupo"})
        fig_t.update_yaxes(tickformat=".3s")
        st.plotly_chart(fig_t, use_container_width=True)

        st.subheader("YoY por grupo")
        g_yoy = q("SELECT * FROM vw_group_yoy WHERE year=? AND quarter=?", (SEL_YEAR, SEL_QTR))
        if True:
            if g_yoy.empty:
                st.info("Sin datos YoY para este período.")
            else:
                g_yoy["GWP MM$"]      = (g_yoy["gwp"] / MM).round(0)
                g_yoy["GWP prev MM$"] = (g_yoy["prev_gwp"] / MM).round(0)
                st.dataframe(
                    g_yoy[["group_name", "GWP MM$", "GWP prev MM$", "gwp_yoy_pct",
                            "loss_ratio_pct", "prev_loss_ratio_pct",
                            "combined_ratio_pct", "prev_combined_ratio_pct"]]
                    .rename(columns={
